...
class ScreenFilter extends PIXI.Filter {
  ...
  */  
  ...
  void main(void){
    ...
    float n = snoise(vec3(uv * 3., sin(time*.2)));
    textureCoord += n * smoothstep(.15, 2., abs(uvm.x)) * .6;
    ...
    vec4 colour = clamp(tex - smoothstep(.5, 1.5, length(uvm)*2.), 0., 1.);

    gl_FragColor = mix(vec4(mix(vec3(.85,.9,.98), vec3(.9,.95,1.), n), 1.), colour, colour.a);
